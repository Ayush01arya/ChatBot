...
const createChatLi = (message, className, isSuggestion = false, imageUrl = null) => {
            ...
            chatContent = `
                <div class="chat-message-content">
                    <div class="chat-image-container">
                        <img src="${imageUrl}" alt="Chat Image" class="chat-image">
                    </div>
                    <p>${message}</p>
                </div>
            `;
    ...
    // Apply background color to incoming messages
    if (className === "incoming") {
        chatLi.querySelector('p')?.classList.add('incoming-message');
    }
    ...
    const sendMessageToFlask = (message) => {
        ...
        return fetch('https://chatwithpdf.astroverse.in/api/generate-response', requestOptions)
...
const handleChat = (message = null) => {
    ...
    sendMessageToFlask(userMessage).then(data => {
        ...
        setTimeout(() => {
            ...
            if (data && data.reply) {
                const imageUrl = data.reply.imageUrl || null;
                const replyMessage = data.reply.message || "";

                chatbox.appendChild(createChatLi(replyMessage, "incoming", false, imageUrl));
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        setTimeout(() => {
            ...
            chatbox.appendChild(createChatLi("Welcome to our chatbot! How can I assist you today?<br><br>"+randomFact, "incoming"));
            chatbox.scrollTo(0, chatbox.scrollHeight);
        }, 20); // 2-second delay
